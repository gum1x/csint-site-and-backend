...
import { generateNewKey, getKeys, deactivateKey, adminLogout } from "./actions"

// Hardcoded admin credentials
const ADMIN_USERNAME = "59dbZ9$0#m?WOHRc"
const ADMIN_PASSWORD = "w#nWy#!ctRr92ZFMLxu0l8J@oEvi"
...
export default function AdminPage() {
  ...
  // Add a loading state for the generate button
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
      ...
      fetchKeys() // Load data on successful login
    } else {
    ...
    if (keys.length > 0) {
      ...
      if (statusFilter !== "all") {
        ...
        if (statusFilter === "active") {
          result = result.filter((key) => key.is_active && new Date(key.expires_at) > now)
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">Select the subscription plan for this key</p>
                  </div>
                    ...
                    <div className="h-full flex items-center justify-center p-6 bg-gray-800/50 rounded-lg border border-gray-700">
                      <div className="text-center">
                        ...
                        <p className="text-gray-500 mt-2 max-w-xs mx-auto">
                          Fill out the form and click "Generate New Key" to create a key for a customer
                        </p>
                      </div>
                    </div>
                  )}
