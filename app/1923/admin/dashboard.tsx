...
import { adminLogout, generateNewApiKey, getApiKeys, deactivateApiKey } from "./actions"
...
export default function AdminDashboard() {
  ...
  const handleGenerateKey = async () => {
    try {
      const result = await generateNewApiKey(planType, expirationDays)
      if (result.success) {
        setNewKey({
          key: result.key,
  ...
  const handleDeactivateKey = async (id: string) => {
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to deactivate API key")
      }
    } catch (err) {
      setError("An unexpected error occurred")
      console.error(err)
                          ...
                          <td className="py-3 px-4 text-sm">
                            <div className="flex items-center">
                              ...
                              {key.redeemed_at
                                ...
                                : `Valid for ${key.duration_days || 30} days after first use`}
                            </div>
                          </td>
                          <td className="py-3 px-4">
                            <span
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-500">Select the plan type for this API key</p>
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="expiration">Duration (days)</Label>
                    <Input
                      id="expiration"
                      type="number"
                      min="1"
                    ...
                    />
                    <p className="text-xs text-gray-500">Number of days the key will be valid after first use</p>
                  </div>
                  ...
                  {newKey ? (
                    <div className="p-6 bg-gray-800 rounded-lg border border-gray-700">
                      ...
                      <div className="flex items-center text-sm text-gray-400 mb-4">
                        ...
                        <span>Valid for {expirationDays} days after first use</span>
                      </div>
